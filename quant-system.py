...
st.set_page_config(layout="wide", page_title="全息量化系統 (V75.9 區間動態版)", initial_sidebar_state="expanded")
...
st.title("全息量化系統 (V75.9 區間動態版)")
user_count, api_limit = get_api_usage(FINMIND_TOKEN)
usage_text = f" | FinMind 額度: {user_count} / {api_limit}" if user_count is not None else ""
st.caption(f"V75.9：新增「全息透視區」自訂區間動態連動，並自動擴展背景天數以確保資料不斷層。{usage_text}")
...
if st.session_state.get('system_running', False):
    ...
    with st.spinner(f"正在啟動 V75.9 終極版決策引擎..."):
        ...
        st.subheader(f"{user_stock_id} {name} 全息戰報 (V75.9 區間動態版)")
        ...
        st.caption(f"備註：所有資料皆已透過 V75.9 動態引擎自動過濾。加權防守價已排除造市高頻刷量誤差。核心分點控盤率為核心券商佔自由流通籌碼之比例，C_Value 最高鎖死於 98%。")
