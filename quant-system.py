    ...
    try: 
        ...
        if r.status_code == 200:
            ...
            if isinstance(data, list) and len(data) > 0:
                ...
                numeric_cols = ['open', 'max', 'min', 'close', 'spread', 'Trading_Volume', 'Trading_money', 'buy', 'sell', 'price']
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=20) as executor:
            ...
            for future in concurrent.futures.as_completed(future_to_type):
                ...
                if f_type == 'branch':
                    ...
                    df_api = pd.DataFrame(data) if data else pd.DataFrame()
                    if not df_api.empty:
                        numeric_cols = ['open', 'max', 'min', 'close', 'spread', 'Trading_Volume', 'Trading_money', 'buy', 'sell', 'price', 'Volume', 'DayTradingVolume']
                        for c in numeric_cols:
                            if c in df_api.columns:
                                df_api[c] = safe_to_num(df_api[c])
                    a_results[ds] = df_api
                ...
                try:
                    ...
                    if r.status_code == 200:
                        ...
                        if isinstance(data, list):
                            for ro in data:
                                if str(ro[0]).replace(' ', '') == str(tid):
                                    vol = safe_to_num(ro[3]) / 1000 
                                    price = safe_to_num(ro[4])
                                    amt = safe_to_num(ro[5]) / 10000 
                                    tt = re.sub(r'<[^>]+>', '', str(ro[2])).strip()
                                    rl.append({"日期": d, "交易別": f"上市-{tt}", "成交量(張)": int(vol), "成交價(元)": price, "成交金額(萬元)": int(amt)})
                except: pass
                try:
                    r = session.get(f"https://www.tpex.org.tw/www/zh-tw/blockTrade/quote?date={dtp}&id=&response=json", timeout=5, verify=False)
                    if r.status_code == 200:
                        tables = r.json().get("tables", [])
                        if tables and isinstance(tables[0].get("data"), list):
                            for ro in tables[0]["data"]:
                                if str(ro[0]).replace(' ', '') == str(tid):
                                    vol = safe_to_num(ro[2]) / 1000 
                                    price = safe_to_num(ro[3])
                                    amt = safe_to_num(ro[4]) / 10000
                                    tt = re.sub(r'<[^>]+>', '', str(ro[5])).strip() if len(ro) > 5 else "上櫃鉅額"
                                    rl.append({"日期": d, "交易別": f"上櫃-{tt}", "成交量(張)": int(vol), "成交價(元)": price, "成交金額(萬元)": int(amt)})
                except: pass
                return rl
...
def scrape_director_v50(tid):
    ...
    except Exception: return {}, 0.0, "系統異常", []
...
def scrape_fubon_pledge(df_pr, tid):
    try:
        ...
        prd = {pd.to_datetime(r['date']).strftime('%Y-%m-%d'): r['close'] for _, r in df_pr.iterrows()} if not df_pr.empty else {}
        pps, mcs = [], []
...
def calculate_dynamic_radar_depth(df_b_raw, dates_list, total_lots, df_price):
    ...
    except Exception: return 15, "安全預設"
...
def process_tdcc(df):
    try:
        ...
        unit_series = df['unit'] if 'unit' in df.columns else pd.Series(0, index=df.index)
        people_series = df['people'] if 'people' in df.columns else pd.Series(0, index=df.index)
        
        df['unit'] = (safe_to_num(unit_series) / 1000).round().astype(int)
        df['people'] = safe_to_num(people_series).astype(int)
        
        dates = sorted(df['date'].unique(), reverse=True)[:15]
        df = df[df['date'].isin(dates)]
        df_levels = df[~df['LevelClean'].astype(str).str.contains('合計|總計')]
...
if run_btn:
    ...
    with st.spinner(f"正在啟動 V60.39 終極裝甲決策引擎..."):
        ...
        if df_p_raw.empty: 
            st.error("查無歷史股價資料，請確認代號是否正確。")
            st.stop()
        
        # 即使 API 回傳正常，也做一次防呆型別轉換
        for c in ['open', 'max', 'min', 'close', 'spread']:
            if c in df_p_raw.columns:
                df_p_raw[c] = safe_to_num(df_p_raw[c])
        
        if 'date' not in df_p_raw.columns:
            st.error("API 回傳資料遺失日期欄位，無法運算。")
            st.stop()
        ...
        # [V60.39 防護盾生效] process_price 不會再因 KeyError 當機
        ...
        st.subheader(f"{user_stock_id} {name} 全息戰報 (V60.39 終極無敵防呆版)")
        ...
        if not df_ta_full.empty:
            ...
            if not df_plot.empty:
                ...
                """
                components.html(
                    html_code.replace("KLINE_DATA", json.dumps(kline_data))\
        ...
        with st.expander(f"給 AI 的 V60.39 實戰精華資料包 (CSV格式)", expanded=True):
