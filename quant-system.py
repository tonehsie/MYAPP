...
st.title("全息量化系統 (V60.35 全域裝甲版)")
user_count, api_limit = get_api_usage(FINMIND_TOKEN)
usage_text = f" | FinMind 額度: {user_count} / {api_limit}" if user_count is not None else ""
st.caption(f"V60.35：全域植入 Type Coercion 防護，徹底消滅 API 回傳字串導致的運算當機問題。{usage_text}")
...
# 【V60.35 全域防護】在取得資料的第一時間強制轉換型別，防範髒字串
...
if run_btn:
    ...
    with st.spinner(f"正在啟動 V60.35 全域防護決策引擎..."):
        ...
        # 【V60.35 全域防護】再次確保 df_p_raw 為純數值，消滅 DataFrame 錯誤
        ...
        st.subheader(f"{user_stock_id} {name} 全息戰報 (V60.35 全域防護版)")
