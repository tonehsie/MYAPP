...
st.set_page_config(layout="wide", page_title="全息量化系統 (V60.56版)", initial_sidebar_state="expanded")
...
st.title("全息量化系統 (V60.56 邏輯無懈可擊版)")
user_count, api_limit = get_api_usage(FINMIND_TOKEN)
usage_text = f" | FinMind 額度: {user_count} / {api_limit}" if user_count is not None else ""
st.caption(f"V60.56：修正真實成本結算盲區，重構聰明錢敵我辨識，導正「隔日沖」過夜雜訊邏輯，確保數據 100% 貼合真實交易。{usage_text}")
...
run_btn = st.button("啟動 V60.56 決策引擎", use_container_width=True, key="run_engine")
...
def scrape_fubon_pledge(df_pr, tid):
    ...
    if df_pr.empty or 'date' not in df_pr.columns or 'close' not in df_pr.columns:
        prd = {}
    else:
    ...
    for r in df_all.itertuples(index=False):
        name = r[2]
        if name not in sm: sm[name] = {"title": r[1], "balance": r[5], "p": "-", "mc": "-"} 
        if sm[name]["p"] == "-" and r[3] > 0:  
            sm[name]["p"] = r[7] 
            sm[name]["mc"] = r[8] 
...
def get_v50_intelligence(df_b_raw, df_p_raw, stick_thresh, global_days, dates_list):
    ...
    df_p.loc[cond_normal, 'pos'] = ((df_p['close'] - df_p['min']) / range_diff)[cond_normal]
    ...
    d5 = set(dates_list[:5])
    d20 = set(dates_list[:20] if len(dates_list) >= 20 else dates_list)
    d60 = set(dates_list[:60] if len(dates_list) >= 60 else dates_list)
...
def calculate_dynamic_radar_depth(df_b_raw, dates_list, total_lots, df_price):
    ...
    df_20 = df_b_raw[df_b_raw['date'].isin(set(dates_list[:20]))].copy()
...
# 【V60.56 修正】實質成本法 (True Cost Basis)，解決高賣低買後出現的幽靈高成本問題
def calculate_pure_defense_line(df_b_raw, tags, is_filter_active, total_lots, dead_chip_ratio, dynamic_n):
    ...
    df = df_b_raw[['securities_trader', 'buy', 'sell', 'valid_buy', 'valid_buy_amt', 'valid_sell_amt']].copy()
    df['tag'] = df['securities_trader'].map(tags).fillna("[隨波逐流]")
    
    if is_filter_active: 
        exclude_tags = {"[短線狙擊]", "[快閃散戶]", "[長線倒貨]"}
        valid_df = df[~df['tag'].isin(exclude_tags)].copy()
    else: 
        valid_df = df
    ...
    broker_stats['net_amt'] = broker_stats['buy_amt'] - broker_stats['sell_amt']
    ...
    top_buyers['true_cost'] = np.where(top_buyers['net_amt'] > 0, top_buyers['net_amt'] / top_buyers['net_vol'].replace(0, np.nan), 0.0)
    valid_top_buyers = top_buyers[top_buyers['true_cost'] > 0]
    total_net_vol = valid_top_buyers['net_vol'].sum()
    
    vwap = round((valid_top_buyers['true_cost'] * valid_top_buyers['net_vol']).sum() / total_net_vol, 2) if total_net_vol > 0 else 0.0
...
def get_core_period_net(df_raw, rank_dates, core_names):
    ...
    df_rank = df_raw[df_raw['date'].isin(set(rank_dates)) & df_raw['securities_trader'].isin(set(core_names))]
...
def process_footprint(df_raw, display_dates, rank_dates, intel_tags, df_fingerprint, top_n):
    ...
    df_rank = df_raw[df_raw['date'].isin(set(rank_dates))]
    ...
    df_disp = df_raw[df_raw['date'].isin(set(display_dates))].copy()
...
def process_branch_v25(df_raw, period, actual_dates, intel_tags, df_price_raw, stick_thresh, global_days):
    ...
    df = df_raw.loc[df_raw['date'].isin(set(actual_dates[:period])), ['securities_trader', 'buy', 'sell', 'valid_buy', 'valid_sell', 'valid_buy_amt', 'valid_sell_amt']].copy()
...
def get_smart_threshold(price, total_lots, dead_float):
    ...
    # 【V60.56 修正】不再把台積電跟銅板股的天花板卡死在 1000，讓演算法更能自動抓出正確大戶級距
    raw_threshold = max(10, min(2000, raw_threshold))
    
    levels = [10, 20, 30, 40, 50, 100, 200, 400, 600, 800, 1000]
    al = min(levels, key=lambda x: abs(x - raw_threshold))
    return al

def process_v27_ultimate_radar(df_wide, parsed_dci, dynamic_dict, static_val, df_price, df_branch_raw, intel_tags):
    ...
    if not df_branch_raw.empty:
        ...
        mask_short = df_b_tagged['tag'].isin({"[短線狙擊]", "[低檔回補]", "[快閃散戶]"})
    ...
    for d in df['日期'].unique():
         cur_dead, _ = get_dead_chip_info(d, parsed_dci, dynamic_dict, static_val, "")
         dead_cache[d] = max(0.0, min(99.9, cur_dead))
    df['safe_dead_ratio'] = df['日期'].map(dead_cache)
    
    df['base_lots'] = 15000 / df['收盤價(元)'].replace(0, np.nan)
    df['free_float_ratio'] = np.maximum(0.05, (100 - df['safe_dead_ratio']) / 100)
    df['float_1pct_lots'] = df['總張數'] * df['free_float_ratio'] * 0.01
    
    df['raw_threshold'] = np.minimum(df['base_lots'], df['float_1pct_lots'])
    df['raw_threshold'] = df['raw_threshold'].clip(10, 2000).fillna(1000)
    
    levels = np.array([100, 200, 400, 600, 800, 1000])
    diffs = np.abs(df['raw_threshold'].values[:, None] - levels)
    ...
    df['prev_large_pct_adj'] = df['current_large_pct'].shift(1)
    df['raw_chg'] = (df['current_large_pct'] - df['prev_large_pct_adj']).round(2)
    ...
    for i, row in df.iterrows():
        ...
        if pd.isna(row['prev_large_pct_adj']):
            p_chg, f_impact = 0.0, 0.0
            adv = ["初始化 (基準建立)"]
            out.append({"日期": d_str, "大戶原持股(%)": round(row['current_large_pct'], 2), "原始大戶變動(%)": 0.0, "純淨變動": p_chg, "雜訊": round(f_impact, 2), "診斷": " | ".join(adv)})
            continue
        ...
        if len(arr_dates_str) > 0:
            ...
            if idx >= 0:
                ...
                if days_diff <= 7 and last_trading_date in fake_dict:
                    for fr in fake_dict[last_trading_date]:
        ...
        adv = []
        if ppl_var > 2.0 and p_chg < 0: adv.append(f"[逃命波] 散戶增{ppl_var}%，大戶實質倒貨{abs(p_chg)}%")
        else:
            if p_chg * lev > 2.5 and p > ma20: adv.append(f"[強勢軋空] 站上月線且大戶純淨買超{round(p_chg*lev, 2)}%")
            elif p_chg > 0.4 and p < ma20: adv.append(f"[底位建倉] 跌破月線但主力吃貨{p_chg}%")
            elif p_chg < -1.0: adv.append(f"[主力撤退] 大戶實質流出{abs(p_chg)}%")
            if f_impact > 1.2: adv.append(f"[隔日沖/短線干擾] 虛胖買盤潛藏{round(f_impact, 2)}%倒貨危機")
            
        out.append({"日期": d_str, "大戶原持股(%)": round(row['current_large_pct'], 2), "原始大戶變動(%)": raw_chg, "純淨變動": p_chg, "雜訊": round(f_impact, 2), "診斷": " | ".join(adv) if adv else "盤整"})
        
    ddf = pd.DataFrame(out)
    df = pd.merge(df, ddf, on='日期', how='left')
    df['專家雷達診斷'] = df['診斷']
    df['純淨大戶變動(%)'] = df['純淨變動']
    # 【V60.56 修正】文字防呆，精準描述干擾來源
    df['隔日沖干擾(%)'] = df['雜訊']
    res_df = df[['日期', '收盤價(元)', '大戶原持股(%)', '總人數變率(%)', '原始大戶變動(%)', '隔日沖干擾(%)', '純淨大戶變動(%)', '專家雷達診斷']].sort_values('日期', ascending=False)
...
def process_branch_diff(df_raw, actual_dates, fire_thresh, period_days=10):
    ...
    df_f = df_raw[df_raw['date'].isin(set(target_dates))]
    if df_f.empty: return pd.DataFrame()

    buy_mask = df_f['buy'] > 0
    sell_mask = df_f['sell'] > 0

    active_cnt = df_f[buy_mask | sell_mask].groupby('date')['securities_trader'].nunique()
    buy_cnt = df_f[buy_mask].groupby('date')['securities_trader'].nunique()
    sell_cnt = df_f[sell_mask].groupby('date')['securities_trader'].nunique()

    buy_vol = df_f[buy_mask].groupby('date')['buy'].sum()
    sell_vol = df_f[sell_mask].groupby('date')['sell'].sum()

    res = pd.DataFrame(index=target_dates)
    res.index.name = 'date'
    
    res['活躍家數'] = active_cnt.reindex(res.index).fillna(0).astype(int)
    res['buy_cnt'] = buy_cnt.reindex(res.index).fillna(0).astype(int)
    res['sell_cnt'] = sell_cnt.reindex(res.index).fillna(0).astype(int)
    res['買賣家數差'] = res['buy_cnt'] - res['sell_cnt']

    res['buy_vol'] = buy_vol.reindex(res.index).fillna(0)
    res['sell_vol'] = sell_vol.reindex(res.index).fillna(0)

    res['籌碼集中度(%)'] = np.where(res['活躍家數'] > 0, (-res['買賣家數差']) / res['活躍家數'] * 100, 0).round(1)

    avg_b = res['buy_vol'] / res['buy_cnt'].replace(0, np.nan)
    avg_s = res['sell_vol'] / res['sell_cnt'].replace(0, np.nan)

    res['買方火力(倍)'] = np.where(avg_s > 0, avg_b / avg_s, np.where(avg_b > 0, 99.9, 1.0)).round(2)

    diag = []
    for d, row in res.iterrows():
        fp = row['買方火力(倍)']
        conc = row['籌碼集中度(%)']
        diff = row['買賣家數差']
        act = row['活躍家數']
        dg = []
        if fp >= fire_thresh and conc > 5: dg.append(f"大戶火力壓制 ({fire_thresh}倍↑)")
        elif fp < 0.7 and diff > 50: dg.append("散戶進場 (主力倒貨)")
        elif act > 500 and fp < 1.0: dg.append("籌碼極度發散 (熱門當沖雷區)")
        diag.append(" | ".join(dg) if dg else "中性換手")

    res['鷹眼診斷'] = diag
    res = res.reset_index().rename(columns={'date': '日期'})
    return res[['日期', '活躍家數', '買賣家數差', '籌碼集中度(%)', '買方火力(倍)', '鷹眼診斷']].sort_values('日期', ascending=False)
...
def process_v30_daily_tracking(df_branch_raw, intel_tags, df_price, df_branch_diff, actual_dates, fire_thresh, period_days=5):
    ...
    df_b = df_branch_raw[df_branch_raw['date'].isin(target_dates)][['date', 'securities_trader', 'valid_buy', 'valid_sell', 'valid_buy_amt', 'valid_sell_amt']].copy()
    
    df_b['tag'] = df_b['securities_trader'].map(intel_tags).fillna("[隨波逐流]")
    
    # 【V60.56 修正】嚴格定義做多的聰明錢，不納入那些長線賣壓，免得死貓反彈造成判讀假象
    smart_mask = df_b['tag'].isin({"[波段鐵粉]", "[常駐造市]"})
    short_mask = df_b['tag'].isin({"[短線狙擊]", "[低檔回補]", "[快閃散戶]"})

    df_smart = df_b[smart_mask].groupby(['date', 'securities_trader', 'tag']).agg(
        bs=('valid_buy','sum'), ss=('valid_sell','sum'), buy_amt=('valid_buy_amt','sum'), sell_amt=('valid_sell_amt','sum')
    ).reset_index()
    df_smart['net_vol'] = ((df_smart['bs'] - df_smart['ss']) / 1000).round().astype(int)
    
    df_short = df_b[short_mask].groupby(['date', 'securities_trader']).agg(
        bs=('valid_buy','sum'), ss=('valid_sell','sum')
    ).reset_index()
    df_short['net_vol'] = ((df_short['bs'] - df_short['ss']) / 1000).round().astype(int)
    
    smart_group = df_smart.groupby('date') if not df_smart.empty else None
    short_group = df_short.groupby('date') if not df_short.empty else None

    smart_net_s = df_smart.groupby('date')['net_vol'].sum()
    short_trap_s = df_short[df_short['net_vol'] > 0].groupby('date')['net_vol'].sum()
    
    df_smart_long = df_smart[df_smart['net_vol'] > 0].copy()
    df_smart_long['net_amt'] = df_smart_long['buy_amt'] - df_smart_long['sell_amt']
    agg_long = df_smart_long.groupby('date').agg(total_n=('net_vol', 'sum'), total_amt=('net_amt', 'sum'))
    agg_long['smart_avg_cost'] = np.where(agg_long['total_n'] > 0, agg_long['total_amt'] / agg_long['total_n'], 0.0)
    ...
    if actual_dates[0] in target_dates and not df_smart.empty:
        d0_smart = df_smart[df_smart['date'] == actual_dates[0]]
        for r in d0_smart.itertuples():
            if r.net_vol != 0:
                audit_smart_money.append({"日期": r.date, "分點": r.securities_trader, "標籤": r.tag, "淨買超(張)": r.net_vol})
    
    for d in actual_dates[:period_days]:
        pr_row = price_dict.get(d, {})
        cp, op, hp, lp = pr_row.get('收盤價(元)', 0), pr_row.get('開盤價(元)', 0), pr_row.get('最高價(元)', 0), pr_row.get('最低價(元)', 0)
        ...
        smart_net = smart_net_s.get(d, 0)
        short_trap = short_trap_s.get(d, 0)
        
        total_n = agg_long.at[d, 'total_n'] if d in agg_long.index else 0
        smart_avg_cost = agg_long.at[d, 'smart_avg_cost'] if d in agg_long.index else 0.0
    ...
    return pd.DataFrame(out), df_audit
...
def process_tdcc(df):
    ...
    dates = sorted(valid_dates.unique(), reverse=True)[:15]
...
def process_tdcc_dynamic(df_share_wide, df_price, parsed_dci, dynamic_dict, static_val, chip_engine):
    ...
    dead_cache, cl_cache = {}, {}
    for d in df_m['日期'].unique():
         cur_dead, cl = get_dead_chip_info(d, parsed_dci, dynamic_dict, static_val, chip_engine)
         dead_cache[d] = max(0.0, min(99.9, cur_dead))
         cl_cache[d] = cl
         
    df_m['safe_dead_ratio'] = df_m['日期'].map(dead_cache)
    df_m['cl'] = df_m['日期'].map(cl_cache)
    
    df_m['base_lots'] = 15000 / df_m['收盤價(元)'].replace(0, np.nan)
    df_m['free_float_ratio'] = np.maximum(0.05, (100 - df_m['safe_dead_ratio']) / 100)
    df_m['float_1pct_lots'] = df_m['總張數'] * df_m['free_float_ratio'] * 0.01
    
    df_m['raw_threshold'] = np.minimum(df_m['base_lots'], df_m['float_1pct_lots'])
    df_m['raw_threshold'] = df_m['raw_threshold'].clip(10, 2000).fillna(1000)
    
    levels = np.array([100, 200, 400, 600, 800, 1000])
    diffs = np.abs(df_m['raw_threshold'].values[:, None] - levels)
    ...
    df_m['cv'] = np.where(df_m['safe_dead_ratio'] < 100,
                          np.maximum(0, (df_m['lp'] - df_m['safe_dead_ratio']) / (100.0 - df_m['safe_dead_ratio'])),
                          0)
    
    conds_st = [df_m['cv'] >= 0.5, df_m['cv'] >= 0.3, df_m['cv'] >= 0.15]
    ...
    out_df['大戶精算門檻'] = "系統判定 (" + df_m['ct'].astype(int).astype(str) + "張)"
    out_df['大戶原持股(%)'] = df_m['lp'].round(2)
    
    mask_dead = df_m['safe_dead_ratio'] > 0
    out_df['董監死籌碼(%)'] = np.where(mask_dead, df_m['safe_dead_ratio'].apply('{:.2f}%'.format) + " (" + df_m['cl'] + ")", "-")
...
def process_fut_inst(df):
    ...
    if 'long_open_interest_balance_volume' not in df.columns:
        df['long_open_interest_balance_volume'] = 0
    if 'short_open_interest_balance_volume' not in df.columns:
        df['short_open_interest_balance_volume'] = 0
...
if run_btn:
    ...
    with st.spinner(f"正在啟動 V60.56 邏輯終極嚴謹引擎..."):
        ...
        for col in ['buy', 'sell', 'price']:
            if col not in df_b_raw.columns:
                df_b_raw[col] = 0.0
        ...
        if not df_v27_radar.empty and not df_s_dyn.empty:
            ...
            if not df_combined_radar.empty:
                ...
                display_cols = ['日期', '收盤價(元)', '純淨活大戶C_Value(%)', '純淨大戶變動(%)', '總人數變率(%)', '大戶精算門檻', '隔日沖干擾(%)', '終極籌碼診斷']
        ...
        df_b_today = process_branch_v25(df_b_raw_60, 1, dates, tags, df_p_raw, stickiness_threshold, max_len)
        df_b_prev1 = process_branch_v25(df_b_raw_60, 1, dates[1:], tags, df_p_raw, stickiness_threshold, max_len)
        df_b_3 = process_branch_v25(df_b_raw_60, 3, dates, tags, df_p_raw, stickiness_threshold, max_len)
        df_b_10 = process_branch_v25(df_b_raw_60, 10, dates, tags, df_p_raw, stickiness_threshold, max_len)
        df_b_60 = process_branch_v25(df_b_raw_60, max_len, dates, tags, df_p_raw, stickiness_threshold, max_len)
        ...
        st.subheader(f"{user_stock_id} {name} 全息戰報 (V60.56)")
        ...
        report_md += "<ul>"
        report_md += f"<li>【防守價與乖離】：系統算出核心主力真實成本為 {vwap_str} 元。今日收盤價 {curr_price} 元，主力帳面乖離率 {bias:.1f}%。</li>\n"
        ...
        elif bias >= 0 and net_60 > 0: layer1_diag = f"股價貼近主力真實成本且{lr_pos_text}，長線大戶默默吸籌，處於安全建倉區。"
        ...
        report_md += "<ul>"
        report_md += f"<li>【Top N 核心主力囤積率】：這群波段大戶吸納了約 {c_val_text} 的市場自由流通籌碼。</li>\n"
        report_md += f"<li>【波段籌碼流向】：排除隔日沖雜訊後，最新一週大戶實質持股 {chg_text}。</li>\n"
        ...
        report_md += f"<li>【今日主力動向】：做多的聰明錢(波段/造市)今日 {dir_smart} {abs(today_smart_net):,} 張。</li>\n"
        ...
        if pat_is_breakdown and today_smart_net < 0:
            ...
            action = "長線底單依然穩固，今日雖有聰明錢流出，但散戶並未瘋狂接刀(家數差未極端發散)。偏向主力順勢調節或刻意壓盤洗浮額。請密切關注股價是否能守住實質防守價，未破線前無須恐慌殺出。"
            ...
            action = "股價雖跌破防守價導致帳面套牢，但今日見到聰明錢進場抵抗。這顯示主力並未完全放棄，正在嘗試逢低護盤。需確認股價能帶量重新站回防守價，才可視為危機解除。"
        else:
        ...
        st.caption(f"備註：所有數據皆已透過 AI 自動過濾。加權防守價為真實成本法(排除獲利了結)。大戶囤積率為核心券商佔自由流通籌碼之比例。")
        ...
        with st.expander(f"給 AI 的 V60.56 實戰精華資料包 (CSV格式)", expanded=True):
