...
st.title("全息量化系統 (V60.40 移除鉅額版)")
user_count, api_limit = get_api_usage(FINMIND_TOKEN)
usage_text = f" | FinMind 額度: {user_count} / {api_limit}" if user_count is not None else ""
st.caption(f"V60.40：已移除鉅額交易模組，確保核心系統全速淨化運行。{usage_text}")
...
def process_branch_diff(df_raw, actual_dates, fire_thresh, period_days=10):
    try:
        ...
        for d in actual_dates[:period_days]:
            ...
            out.append({"日期": d, "活跃家數": active_count, "買賣家數差": diff_count, "籌碼集中度(%)": round(concentration, 1), "買方火力(倍)": round(firepower, 2), "鷹眼診斷": " | ".join(diag) if diag else "中性換手"})
        return pd.DataFrame(out)
    except Exception: return pd.DataFrame()
...
if run_btn:
    ...
    with st.spinner(f"正在啟動 V60.39 終極裝甲決策引擎..."):
        ...
        st.subheader(f"{user_stock_id} {name} 全息戰報 (V60.39)")
