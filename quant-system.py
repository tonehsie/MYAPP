...
st.set_page_config(layout="wide", page_title="全息量化系統 (V71.03版)", initial_sidebar_state="expanded")
...
st.title("全息量化系統 (V71.03 深度智庫版)")
user_count, api_limit = get_api_usage(FINMIND_TOKEN)
usage_text = f" | FinMind 額度: {user_count} / {api_limit}" if user_count is not None else ""
st.caption(f"V71.03：熱力圖全幅展開不裁切，AI 診斷引擎導入深度實戰語義解析，足跡預設更新為 20 日。{usage_text}")
...
run_btn = st.button("啟動 V71.03 決策引擎", use_container_width=True, key="run_engine")
...
# 💡 V71.03 更新：熱力圖去除 max-height 限制，改為 none 讓所有行數完整展開
def render_footprint_heatmap(df_raw, display_dates, rank_dates, intel_tags, top_n, noise_threshold):
    ...
    # 這裡將 max-height 改為 none !important 覆寫 CSS，保證全開
...
if run_btn:
    ...
    with st.spinner(f"正在啟動 V71.03 深度智庫版決策引擎..."):
        ...
        st.subheader(f"{user_stock_id} {name} 全息戰報 (V71.03)")
        ...
        st.caption(f"備註：所有數據皆已透過 V71.03 動態引擎自動過濾。加權防守價已排除造市高頻刷量誤差。核心分點控盤率為核心券商佔自由流通籌碼之比例，C_Value 最高鎖死於 98%。")
        ...
        with st.expander(f"給 AI 的 V71.03 實戰精華資料包 (CSV格式)", expanded=True):
        ...
        st.success(f"V71.03 已成功處理 {user_stock_id}。當前 RAM 使用狀態健康。")
        gc.collect()

st.divider()
st.caption("V71.03 備註：此版本專為 1GB RAM 環境優化，解除熱力圖高度限制，並導入 AI 深度實戰語義解析引擎。")
