...
# 設定網頁標題與佈局
st.set_page_config(page_title="V26.3 終極全息量化系統", layout="wide")
...
.info-box { background-color: #f0f2f6; padding: 15px; border-radius: 10px; margin-bottom: 20px; border-left: 5px solid #ff4b4b; }
</style>
""", unsafe_allow_html=True)

st.title("🤖 交易員實戰手冊：V26.3 全息量化除水系統")
st.caption("核心升級：導入 FinMind 基本面地緣情報，並強化雷達診斷之實戰敘述。")
...
run_btn = st.button("🚀 啟動 V26.3 引擎：擷取地緣情報與全息雷達", use_container_width=True)
...
@st.cache_data(ttl=3600, show_spinner=False)
def get_company_basic_info(target_id):
    """擷取 FinMind 產業類別與 Yahoo 基本面地址供地緣研判"""
    industry = "未知產業"
    address = "查無地址"
    # 1. FinMind 抓產業
    try:
        ...
        if not fm_info.empty and 'stock_id' in fm_info.columns:
            match_row = fm_info[fm_info['stock_id'] == target_id]
            if not match_row.empty:
                industry = match_row['industry_category'].iloc[0]
    except: pass
    
    # 2. Yahoo 抓精確地址 (因 FinMind TaiwanStockInfo 無精確地址欄位)
    try:
        res = requests.get(f"https://tw.stock.yahoo.com/quote/{target_id}/profile", headers={"User-Agent": "Mozilla/5.0"}, timeout=5)
        addr_match = re.search(r'公司地址.*?<span[^>]*>([^<]+)</span>', res.text, re.IGNORECASE)
        if addr_match: address = addr_match.group(1).strip()
    except: pass
    
    return f"🏢 **【所屬產業】** {industry} ｜ 📍 **【公司地址 (地緣核對用)】** {address}"
...
def process_v25_ultimate_radar(df_wide, dead_chip_input, dynamic_dict, static_val, df_price, df_branch_raw, intel_tags):
    ...
    for i, row in df.iterrows():
        if pd.isna(row['1000張變動(%)']):
            out_diag.append({"真實變動": 0, "雜訊": 0, "診斷": "⚪ 數據初始化階段..."}); continue
        ...
        # 【修改重點】：生成詳細的實戰診斷文字
        advice = []
        if row['總人數變動率(%)'] > 2.0 and pure_chg < 0: 
            advice.append(f"💀 [逃命警報] 散戶暴增 {row['總人數變動率(%)']}% 接刀，大戶真實籌碼流出 {abs(pure_chg)}%")
        else:
            if pure_chg * lev > 2.5 and row['收盤價(元)'] > row['ma20']: 
                advice.append(f"🚀 [真·暴力軋空] 站上月線且大戶實質爆買，純淨推升力道達 {round(pure_chg * lev, 2)}%")
            elif pure_chg > 0.4 and row['收盤價(元)'] < row['ma20']: 
                advice.append(f"🧱 [低檔建倉] 股價處於月線之下，但大戶已暗中吃貨 {pure_chg}%")
            elif pure_chg < -1.0:
                advice.append(f"📉 [主力撤退] 大戶真實籌碼明顯流出 {abs(pure_chg)}%")
                
            if f_impact > 1.2: 
                advice.append(f"⚡ [隔日沖陷阱] 買盤虛胖，潛藏 {round(f_impact, 2)}% 隔日沖倒貨危機")
        
        final_advice = " | ".join(advice) if advice else "🔵 趨勢盤整/無明顯方向訊號"
        out_diag.append({"真實變動": pure_chg, "雜訊": round(f_impact, 2), "診斷": final_advice})

    diag_df = pd.DataFrame(out_diag)
    df['真實大戶變動(%)'], df['隔日沖雜訊(%)'], df['V26.3_專家診斷'] = diag_df['真實變動'], diag_df['雜訊'], diag_df['診斷']
    return df[['日期', '收盤價(元)', '總人數變動率(%)', '1000張變動(%)', '真實大戶變動(%)', '隔日沖雜訊(%)', 'V26.3_專家診斷']].sort_values('日期', ascending=False), pd.DataFrame(debug_math), pd.DataFrame(debug_friday)
...
def process_v26_daily_tracking(df_branch_raw, intel_tags, df_price, df_branch_diff, actual_dates):
    ...
    for d in actual_dates[:5]:
        p_row = df_price[df_price['日期'] == d]
        close_p = p_row['收盤價(元)'].iloc[0] if not p_row.empty else 0
        spread = p_row['漲跌(元)'].iloc[0] if not p_row.empty else 0
        
        diff_row = df_branch_diff[df_branch_diff['日期'] == d]
        b_s_diff = diff_row['買賣家數差'].iloc[0] if not diff_row.empty else 0
        
        df_d = df_branch_raw[df_branch_raw['date'] == d]
        smart_net, noise_net = 0, 0
        if not df_d.empty:
            df_d = df_d.copy()
            df_d['tag'] = df_d['securities_trader'].map(intel_tags).fillna("🔵 一般")
            df_d['net_vol'] = (df_d['buy'] - df_d['sell']) / 1000
            smart_df = df_d[df_d['tag'].str.contains('波段主|真鎖碼|官股')]
            smart_net = smart_df['net_vol'].sum()
            noise_df = df_d[df_d['tag'].str.contains('隔日沖')]
            noise_net = noise_df['net_vol'].sum()
            
        advice = []
        if smart_net > 50 and b_s_diff < 0: advice.append("🟢 主力吃貨/籌碼集中")
        elif smart_net < -100: advice.append("🔴 聰明錢撤退")
        if noise_net > 300: advice.append("⚠️ 隔日沖進駐")
        if b_s_diff > 100: advice.append("📉 散戶進場/籌碼發散")
        
        out.append({
            "日期": d, "收盤價(元)": close_p, "漲跌(元)": spread,
            "聰明錢淨流(張)": int(smart_net), "隔日沖留倉(張)": int(noise_net),
            "買賣家數差": b_s_diff, "平日獨立診斷": " | ".join(advice) if advice else "無明顯特徵"
        })
...
def process_tdcc(df):
    ...
    p_ppl = df_levels.pivot_table(index='date', columns='LevelClean', values='people', aggfunc='first').reset_index().fillna(0)
    ...
    df_total['總人數(人)'] = p_ppl[lvls].sum(axis=1)
    ...
    df_ppl_out = pd.merge(df_total[['date', '總人數(人)']], p_ppl[['date']+lvls], on='date').rename(columns={'date': '日期'}).sort_values('日期', ascending=False)
    
    return df_wide, df_unit, df_ppl_out
...
def process_branch_v25(df_raw, period, actual_dates, intel_tags):
    ...
    for i in range(15):
        row = {}
        if i < len(b): row["買超分點"] = f"{intel_tags.get(b.loc[i,'securities_trader'],'🔵')} {b.loc[i,'securities_trader']}"; row["買超(張)"] = int(b.loc[i,'net']); row["佔比"] = f"{(b.loc[i,'net']/total_buy)*100:.1f}%"
        else: row["買超分點"] = "-"; row["買超(張)"] = 0; row["佔比"] = "-"
        if i < len(s): row["賣超分點"] = f"{intel_tags.get(s.loc[i,'securities_trader'],'🔵')} {s.loc[i,'securities_trader']}"; row["賣超(張)"] = abs(int(s.loc[i,'net'])); row["佔比_"] = f"{(abs(s.loc[i,'net'])/total_buy)*100:.1f}%"
...
def show_table(title, df, custom_class=""):
    ...
    if df is None or df.empty: 
        ...
        left_cols = [c for c in df.columns if any(kw in str(c) for kw in ['日期', '分點', '名稱', '姓名', '身份別', '質權人', '交易別', '診斷', '判定', '門檻', '條件', '措施', '契約', '代號', '來源', '標籤', '平日獨立診斷'])]
...
# ==========================================
if run_btn:
    with st.spinner(f"正在執行 V26.3 全息除水引擎與地緣情報掃描..."):
        name = get_stock_name(user_stock_id)
        
        # 【新增】：抓取公司產業與地址 (顯示於最前方)
        company_info_text = get_company_basic_info(user_stock_id)
        ...
        # --- 頁面呈現 ---
        st.subheader(f"📊 {user_stock_id} {name} V26.3 全息戰報")
        
        # 【新增】：在平日戰情矩陣前顯示地址與產業資訊
        st.markdown(f"<div class='info-box'>{company_info_text}</div>", unsafe_allow_html=True)
        st.caption("※ 實戰應用：請核對上方的『公司地址』與下方的『波段主力分點』，若兩者位於同一個縣市/區域，即為高勝率的【📍 地緣公司派】。")
        
        show_table("⚡ 0. 平日戰情追蹤矩陣 (週一至週四核心代理指標)", df_daily_tracker, "daily-tracker")
        
        show_table("1-1. 雙軸活大戶鎖碼判定表 (C-Value) (近8週)", df_s_dyn)
        show_table("1-2. V26.3 專家診斷雷達 (週末除水版) (近8週)", df_v26_radar, "radar-table")
        ...
        # 稽核中心
        with st.expander("🛠️ 【開發者專用】V26.3 演算法稽核中心", expanded=False):
        ...
        # AI 戰報生成 (CSV 格式)
        st.divider()
        with st.expander("📋 【點擊展開：給 Gemini 的 V26.3 量化分析與稽核資料包 (CSV格式)】", expanded=True):
            p = f"請分析標的: {user_stock_id} {name} (V26.3 量化籌碼)\n"
            ...
            p += format_to_csv_string(df_v26_radar.head(8), "1-2. V26.3 專家診斷雷達 (週末除水版)")
            ...
            p += "\n請幫我驗證以上 CSV 數據的邏輯正確性，結合『0.平日戰情追蹤矩陣』與公司地址地緣性，給出明天與下週的操作建議。"
