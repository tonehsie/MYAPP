...
st.set_page_config(layout="wide", page_title="全息量化系統 (V60.52版)", initial_sidebar_state="expanded")
...
# 【V60.52 優化】靜態預載矩陣，消滅啟動初期的字典推導負擔
_LEVEL_CLEAN_CACHE = {
    "1": "1-999股", "2": "1-5張", "3": "5-10張", "4": "10-15張", "5": "15-20張",
    "6": "20-30張", "7": "30-40張", "8": "40-50張", "9": "50-100張", "10": "100-200張",
...
st.title("全息量化系統 (V60.52 終極防呆防崩潰版)")
user_count, api_limit = get_api_usage(FINMIND_TOKEN)
usage_text = f" | FinMind 額度: {user_count} / {api_limit}" if user_count is not None else ""
st.caption(f"V60.52：全面防禦 NaN 引發的 TypeError 崩潰，導入靜態快取與原生 Pandas 對射加速，消滅空轉效能。{usage_text}")
...
run_btn = st.button("啟動 V60.52 決策引擎", use_container_width=True, key="run_engine")
...
def process_tdcc_dynamic(df_share_wide, df_price, parsed_dci, dynamic_dict, static_val, chip_engine):
    ...
    out_df['董監死籌碼(%)'] = np.where(mask_dead, df_m['safe_dead_ratio'].apply(lambda x: f"{x:.2f}%") + " (" + df_m['cl'] + ")", "-")
...
if run_btn:
    ...
    with st.spinner(f"正在啟動 V60.52 終極防護引擎..."):
        ...
        # 【V60.52 優化】核心防護：確保日期的 NaN 徹底被剔除，防止 sorted() 觸發 TypeError '<' not supported 崩潰
        if df_p_raw.empty or 'date' not in df_p_raw.columns: 
            st.error("查無歷史股價資料或資料異常。")
            st.stop()
        ...
        st.subheader(f"{user_stock_id} {name} 全息戰報 (V60.52)")
        ...
        with st.expander(f"給 AI 的 V60.52 實戰精華資料包 (CSV格式)", expanded=True):
