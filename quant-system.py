...
st.set_page_config(layout="wide", page_title="全息量化系統 (V71.12.5 老手盤感升級版)", initial_sidebar_state="expanded")
...
st.title("全息量化系統 (V71.12.5 老手盤感升級版)")
user_count, api_limit = get_api_usage(FINMIND_TOKEN)
usage_text = f" | FinMind 額度: {user_count} / {api_limit}" if user_count is not None else ""
st.caption(f"V71.12.5：核心重構加入大戶持股甜區篩選、處置股預測及主力成交力防騙線機制。{usage_text}")
    ...
    try:
        ...
        for row in df.to_dict('records'):
            ...
            if prev_row is None:
                ...
                adv =
                if row.get('總人數變率(%)', 0) > 2.0 and p_chg < 0: adv.append(f"散戶增{row.get('總人數變率(%)', 0)}%，大戶實質倒貨{abs(p_chg)}%")
                else:
                    if p_chg * lev > 2.5 and row.get('收盤價(元)', 0) > row.get('ma20', 0): adv.append(f"站上月線且大戶純淨買超{round(p_chg*lev, 2)}%")
                    elif p_chg > 0.4 and row.get('收盤價(元)', 0) < row.get('ma20', 0): adv.append(f"跌破月線但主力吃貨{p_chg}%")
                    elif p_chg < -1.0: adv.append(f"大戶實質流出{abs(p_chg)}%")
                    if f_impact > 1.2: adv.append(f"虛胖買盤潛藏{round(f_impact, 2)}%倒貨危機")
...
def process_tdcc(df):
    ...
    if 'HoldingShares' in df.columns:
        df['unit'] = (safe_to_num(df) / 1000).round().astype(int)
...
def process_inst(df):
    ...
    length = len(pdf)
    f_b = safe_to_num(pdf.get('buy_Foreign_Investor', pd.Series(*length)))
    f_s = safe_to_num(pdf.get('sell_Foreign_Investor', pd.Series(*length)))
    out['外資買賣超(張)'] = ((f_b - f_s) / 1000).round().astype(int)
    i_b = safe_to_num(pdf.get('buy_Investment_Trust', pd.Series(*length)))
    i_s = safe_to_num(pdf.get('sell_Investment_Trust', pd.Series(*length)))
    out['投信買賣超(張)'] = ((i_b - i_s) / 1000).round().astype(int)
    ds_b = safe_to_num(pdf.get('buy_Dealer_self', pdf.get('buy_Dealer', pd.Series(*length))))
    ds_s = safe_to_num(pdf.get('sell_Dealer_self', pdf.get('sell_Dealer', pd.Series(*length))))
    out['自營商(自行)買賣超(張)'] = ((ds_b - ds_s) / 1000).round().astype(int)
    dh_b = safe_to_num(pdf.get('buy_Dealer_Hedging', pd.Series(*length)))
    dh_s = safe_to_num(pdf.get('sell_Dealer_Hedging', pd.Series(*length)))
    ...
    if "標的股價(元)" in df_out.columns and "轉換價(元)" in df_out.columns:
        ...
        if "溢價率(%)" not in df_out.columns or df_out["溢價率(%)"].isna().all():
            if "CB收盤價" in df_out.columns and "轉換價值" in df_out.columns:
                ...
                df_out["溢價率(%)"] = ((df_out - df_out["轉換價值"]) / df_out["轉換價值"] * 100).round(2)
            else: df_out["溢價率(%)"] = "-"
...
def calculate_disposition_thresholds(df_price, df_day_trade, total_lots):
    ...
    res = {}
    res['limit_6d'] = closes[-6] * 1.32 if len(closes) >= 6 else None
        ...
        if "三重底" in mode or is_auto:
            if len(lows) >= 3:
                ...
                if l1[1] > 0 and l2[1] > 0 and abs(l1[1]-l2[1])/l1[1] < tol and abs(l2[1]-l3[1])/l2[1] < tol:
                    b_h = [h for h in highs if l1[3] < h[3] < l3[3]]
                    if b_h:
                        h_max = max(b_h, key=lambda x: x[1])
                        status = "已突破頸線" if current_price > h_max[1] else "成型中"
                        return {
                            'name': '三重底', 'shape_x': [l1, b_h, l2, b_h[-1], l3], 'shape_y': [l1[1], b_h[1], l2[1], b_h[-1][1], l3[1]],
                            'neck_x': [l1, last_date], 'neck_y': [h_max[1], h_max[1]], 'color': '#9c27b0', 'desc': f"三重底 ({status})", 'signal': 'bullish'
                        }
        ...
        if "三重頂" in mode or is_auto:
            if len(highs) >= 3:
                ...
                if h1[1] > 0 and h2[1] > 0 and abs(h1[1]-h2[1])/h1[1] < tol and abs(h2[1]-h3[1])/h2[1] < tol:
                    b_l = [l for l in lows if h1[3] < l[3] < h3[3]]
                    if b_l:
                        l_min = min(b_l, key=lambda x: x[1])
                        status = "已跌破頸線" if current_price < l_min[1] else "成型中"
                        return {
                            'name': '三重頂', 'shape_x': [h1, b_l, h2, b_l[-1], h3], 'shape_y': [h1[1], b_l[1], h2[1], b_l[-1][1], h3[1]],
                            'neck_x': [h1, last_date], 'neck_y': [l_min[1], l_min[1]], 'color': '#d32f2f', 'desc': f"三重頂 ({status})", 'signal': 'bearish'
                        }
        ...
        if "頭肩底" in mode or is_auto:
            if len(lows) >= 3:
                ...
                if l1[1] > 0 and l2[1] < l1[1] and l2[1] < l3[1] and abs(l1[1]-l3[1])/l1[1] < 0.05: 
                    b_h1 = [h for h in highs if l1[3] < h[3] < l2[3]]
                    b_h2 = [h for h in highs if l2[3] < h[3] < l3[3]]
                    if b_h1 and b_h2:
                        h1, h2 = max(b_h1, key=lambda x: x[1]), max(b_h2, key=lambda x: x[1])
                        status = "已突破頸線" if current_price > max(h1[1], h2[1]) else "打右肩中"
                        return {
                            'name': '頭肩底', 'shape_x': [l1, h1, l2, h2, l3], 'shape_y': [l1[1], h1[1], l2[1], h2[1], l3[1]],
                            'neck_x': [h1, last_date], 'neck_y': [h1[1], h2[1]], 'color': '#e91e63', 'desc': f"頭肩底 ({status})", 'signal': 'bullish'
                        }
        ...
        if "頭肩頂" in mode or is_auto:
            if len(highs) >= 3:
                ...
                if h1[1] > 0 and h2[1] > h1[1] and h2[1] > h3[1] and abs(h1[1]-h3[1])/h1[1] < 0.05: 
                    b_l1 = [l for l in lows if h1[3] < l[3] < h2[3]]
                    b_l2 = [l for l in lows if h2[3] < l[3] < h3[3]]
                    if b_l1 and b_l2:
                        l1, l2 = min(b_l1, key=lambda x: x[1]), min(b_l2, key=lambda x: x[1])
                        status = "已跌破頸線" if current_price < min(l1[1], l2[1]) else "做右肩中"
                        return {
                            'name': '頭肩頂', 'shape_x': [h1, l1, h2, l2, h3], 'shape_y': [h1[1], l1[1], h2[1], l2[1], h3[1]],
                            'neck_x': [l1, last_date], 'neck_y': [l1[1], l2[1]], 'color': '#d32f2f', 'desc': f"頭肩頂 ({status})", 'signal': 'bearish'
                        }

        if "W底" in mode or is_auto:
            if len(lows) >= 2:
                l1, l2 = lows[-2], lows[-1]
                between_highs = [h for h in highs if l1[3] < h[3] < l2[3]]
                if between_highs and l1[1] > 0:
                    ...
                    if diff <= tol or "W底" in mode:
                        ...
                        return {
                            'name': 'W底', 'shape_x': [l1, h1, l2], 'shape_y': [l1[1], h1[1], l2[1]],
                            'neck_x': [l1, last_date], 'neck_y': [h1[1], h1[1]], 'color': '#9c27b0', 'desc': desc, 'signal': 'bullish'
                        }

        if "M頭" in mode or is_auto:
            if len(highs) >= 2:
                h1, h2 = highs[-2], highs[-1]
                between_lows = [l for l in lows if h1[3] < l[3] < h2[3]]
                if between_lows and h1[1] > 0:
                    ...
                    if diff <= tol or "M頭" in mode:
                        ...
                        return {
                            'name': 'M頭', 'shape_x': [h1, l1, h2], 'shape_y': [h1[1], l1[1], h2[1]],
                            'neck_x': [h1, last_date], 'neck_y': [l1[1], l1[1]], 'color': '#d32f2f', 'desc': desc, 'signal': 'bearish'
                        }
        ...
        if any(k in mode for k in ["連續", "三角形", "楔形", "矩形"]) or is_auto:
            if len(highs) >= 2 and len(lows) >= 2:
                ...
                if p_name or not is_auto:
                    ...
                    return {'name': p_name, 'shape_x': [h1, h2], 'shape_y': [h1[1], h2[1]], 'neck_x': [l1, l2], 'neck_y': [l1[1], l2[1]], 'color': p_color, 'desc': p_desc, 'signal': p_sig}
                    
        if "V型反轉" in mode or is_auto:
            if len(lows) >= 1 and len(highs) >= 2:
                l1 = lows[-1]
                h_before = [h for h in highs if h[3] < l1[3]] 
                h_after = [h for h in highs if h[3] > l1[3]]
                if h_before and h_after and l1[1] > 0:
                    hb, ha = h_before[-1], h_after
...
if run_btn:
    ...
    with st.spinner(f"正在啟動 V71.12.5 終極解鎖版決策引擎..."):
        ...
        # 這裡將 df_day_trade 的解析往上移，供新模組使用
        ...
        if not df_daily_tracker.empty:
            today_smart_net = df_daily_tracker.iloc.get('聰明錢淨流(張)', 0)
            today_short_trap = df_daily_tracker.iloc.get('潛在賣壓(張)', 0)
            gap_raw = df_daily_tracker.iloc.get('均價落差', 0)
        ...
        if not df_b_diff.empty:
            today_fp = df_b_diff.iloc.get('買方火力(倍)', 1.0)
            today_diff_cnt = df_b_diff.iloc.get('買賣家數差', 0)
        ...
        if not df_combined_display.empty:
            try: 
                c_val_raw = df_combined_display.iloc.get('純淨活大戶C_Value(%)', 0)
                if str(c_val_raw).strip() == "-":
                    c_val_text = f"{df_combined_display.iloc.get('大戶原持股(%)', 0)}% (原始大戶比例)"
                else:
            ...
            try: 
                radar_chg = float(str(df_combined_display.iloc.get('純淨大戶變動(%)', 0)).replace('+', '').replace(',', '').replace('%', '').strip())
        ...
        if not df_ta_full.empty:
            ...
            if not df_plot.empty:
                ...
                if not df_lr_channel.empty:
                    ...
                    lr_data = {
                        "upper": [{"time": str(t), "value": float(v) if pd.notna(v) else 0.0} for t, v in zip(df_plot_lr['日期'], df_plot_lr)],
                        "mid": [{"time": str(t), "value": float(v) if pd.notna(v) else 0.0} for t, v in zip(df_plot_lr['日期'], df_plot_lr)],
                        "lower": [{"time": str(t), "value": float(v) if pd.notna(v) else 0.0} for t, v in zip(df_plot_lr['日期'], df_plot_lr)]
                    }
...
| tvVal === undefined) return;
        ...
        inst_net_today = df_inst.iloc['三大法人買賣超(張)'] if not df_inst.empty else 0
        ...
        if not df_margin.empty and '融資餘額(萬元)' in df_margin.columns and len(df_margin) > 1:
            today_margin_chg = safe_to_num(df_margin.iloc['融資餘額(萬元)']) - safe_to_num(df_margin.iloc[1]['融資餘額(萬元)'])
        ...
        if not df_cbas.empty and '未償還餘額' in df_cbas.columns and len(df_cbas) >= 2:
            try:
                cb_curr = float(df_cbas.iloc['未償還餘額'])
        ...
        # 插入新的監管套利預警判斷
        if disp_warn:
            if disp_warn.get('day_trade_warning'):
                report_md += "<li style='color:#d32f2f; font-weight:bold;'>⚠️ 【短線過熱警告】：近兩日當沖佔比逾60%，依據法規即將觸發注意股紅線，須防範流動性枯竭風險。</li>"
            if disp_warn.get('turnover_warning'):
                report_md += "<li style='color:#d32f2f; font-weight:bold;'>⚠️ 【高週轉率警告】：近5日週轉率極端過高(>10%)，已達投機標準，隨時可能進入處置分盤交易。</li>"
            if disp_warn.get('limit_6d') and curr_price > disp_warn['limit_6d']:
                report_md += "<li style='color:#d32f2f; font-weight:bold;'>⚠️ 【漲幅超限警告】：近6日累積漲幅達32%，觸發處置股警戒線，主力極可能被迫提早洗盤降溫。</li>"
        ...
        report_md += f"<div class='ai-conclusion'><b>🚀 最終操作定調：{conclusion}</b><br><span style='font-weight:normal; font-size:1.05rem; display:block; margin-top:8px;'>{action}</span></div>\n"
        report_md += "</div>"
        
        st.markdown(report_md, unsafe_allow_html=True)
        st.caption(f"備註：所有數據皆已透過 V71.12.5 動態引擎自動過濾。加權防守價已排除造市高頻刷量誤差。核心分點控盤率為核心券商佔自由流通籌碼之比例，C_Value 最高鎖死於 98%。")
        ...
        with st.expander(f"主力分點 - 今日 ({dates})"):
...
st.divider()
st.caption("V71.12.5 備註：核心模組升級，新增大戶甜區、處置紅線及主力買賣力防禦。")    .progress-text { color: #64b5f6 !important; }
    .profit-warning { background-color: #4a148c !important; color: #e1bee7 !important; border-color: #7b1fa2 !important; }
    .loss-warning { color: #ff7043 !important; }
    .highlight-red { color: #ef5350 !important; }
    .highlight-green { color: #66bb6a !important; }
...
def safe_to_num(series, fill_val=0):
    if isinstance(series, pd.Series):
        ...
        if valid_mask.any():
            ...
            failed_mask = temp_converted.isna() & valid_str_mask
            
            if failed_mask.any():
                col_name = series.name if hasattr(series, 'name') and series.name else "未知資料行"
                failed_count = failed_mask.sum()
                sample_val = series[valid_mask][failed_mask].iloc[0]
