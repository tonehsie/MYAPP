...
st.set_page_config(page_title="V47.2 全息量化系統 (動態大腦相容版)", layout="wide", initial_sidebar_state="expanded")
FINMIND_TOKEN = "eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.eyJkYXRlIjoiMjAyNi0wNC0xMCAyMDoyMDo0NiIsInVzZXJfaWQiOiJUb25lMSIsImVtYWlsIjoidG9uZWhzaWVAZ21haWwuY29tIiwiaXAiOiI2MS42Mi43LjE5OCJ9.7s3-IrkfdiUyTvGiZQGESBUBAPHQTnd4pwYcn8_J-CY"

# 將 CSS 改為單行字串拼接，避免 Streamlit 快取解析發生 TokenError
CSS = (
    "<style>"
...
st.sidebar.divider()
st.sidebar.markdown("### 🧠 V47.2 淨化籌碼引擎")
...
st.title("📱 V47.2 終極全息量化系統 (動態大腦相容版)")
user_count, api_limit = get_api_usage(FINMIND_TOKEN)
usage_text = f" | 🔑 FinMind 額度使用狀態: {user_count} / {api_limit}" if user_count is not None else ""
st.caption(f"🚀 V47.2 升級：優化黑白K線粗細、凸顯董監事持股、正確Raw Data Dump。{usage_text}")
...
run_btn = st.button("🚀 啟動 V47.2 決策引擎", use_container_width=True, key="run_engine")
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=3) as ex:
        for data in ex.map(fd, td):
            if data: 
                bd.extend(data)
    if not bd: 
        return pd.DataFrame(), list(set(dl))
    p = []
    for i in bd:
        date, src, row = i
        nums = []
        ...
        if html:
            ...
            if tm:
                ...
                if 0 < sum(ed.values()) < 100: 
                    return {}, round(sum(ed.values()), 2), "富邦精算(備援)", []
    except:
        pass
...
def get_company_profile(tid):
    ...
    try:
        ...
        if not f.empty and 'stock_id' in f.columns:
            ...
            if not m.empty: 
                ind = m['industry_category'].iloc[0]
...
def get_dead_chip_info(ds, dci, dd, sv, ce):
    if dci and str(dci).strip() != "":
        try: 
            return float(str(dci).replace('%', '').strip()), "手動輸入"
        except:
            pass
    mk = str(ds)[:7].replace('/', '-')
    if dd and mk in dd: 
...
def scrape_fubon_pledge(df_pr, tid):
    ...
    for ds in df_all['日期']:
        if len(ds) == 5 and '/' in ds: 
            m = int(ds.split('/')[0])
            if pm == 99: 
                py = cy - 1 if m > cm + 1 and cm < 3 else cy
            elif m > pm + 1: 
                py -= 1
            pm = m
    ...
    for _, r in df_all.iterrows():
        if r['姓名'] not in sm: 
            sm[r['姓名']] = {"title": r['身份別'], "balance": r['累積質設(張)'], "p": "-", "mc": "-"}
...
def get_v47_intelligence(df_b_raw, df_p_raw, stick_thresh, global_days, dates_list):
    if df_b_raw.empty or df_p_raw.empty: 
        return {}, pd.DataFrame()
    if global_days <= 0: 
        global_days = 1
    ...
    for trader, g in df.groupby('securities_trader'):
        tb = round(g['buy'].sum() / 1000)
        ts = round(g['sell'].sum() / 1000)
        tv = tb + ts
        if tv == 0: 
            continue
        ...
        if avg_b > latest_close and avg_b > 0 and (tb-ts) > 0: 
            b_str = f"⚠️(虧) {b_str}"
...
def process_footprint(df_raw, dynamic_dates, intel_tags, df_fingerprint, top_n, global_days):
    if df_raw.empty or not dynamic_dates: 
        return pd.DataFrame(), pd.DataFrame()
...
def process_branch_v25(df_raw, period, actual_dates, intel_tags, df_price_raw, stick_thresh, global_days):
    if df_raw.empty or df_price_raw.empty: 
        return pd.DataFrame()
    ...
    df['ba'] = df['buy'] * df['price']
    df['sa'] = df['sell'] * df['price']
...
def process_v27_ultimate_radar(df_wide, dead_chip_input, dynamic_dict, static_val, df_price, df_branch_raw, intel_tags):
    ...
    for i, row in df.iterrows():
        ...
        adv = []
        if row['總人數變動率(%)'] > 2.0 and p_chg < 0: 
            adv.append(f"💀 [逃命] 散戶增{row['總人數變動率(%)']}%，大戶實質倒貨{abs(p_chg)}%")
        else:
            if p_chg * lev > 2.5 and row['收盤價(元)'] > row['ma20']: 
                adv.append(f"🚀 [真軋空] 站上月線且大戶純淨買超{round(p_chg*lev, 2)}%")
            elif p_chg > 0.4 and row['收盤價(元)'] < row['ma20']: 
                adv.append(f"🧱 [底位建倉] 跌破月線但主力吃貨{p_chg}%")
            elif p_chg < -1.0: 
                adv.append(f"📉 [主力撤退] 大戶實質流出{abs(p_chg)}%")
            if f_impact > 1.2: 
                adv.append(f"⚡ [隔日沖陷阱] 虛胖買盤潛藏{round(f_impact, 2)}%倒貨危機")
...
def process_branch_diff(df_raw, actual_dates, fire_thresh):
    if df_raw.empty or not actual_dates: 
        return pd.DataFrame()
    out = []
...
def process_v30_daily_tracking(df_branch_raw, intel_tags, df_price, df_branch_diff, actual_dates, fire_thresh):
    if df_branch_raw.empty or len(actual_dates) < 5: 
        return pd.DataFrame(), pd.DataFrame()
    out = []
    audit_smart_money = []
    ...
    for d in actual_dates[:5]:
        ...
        if day_range > 0 and (lower_shadow / day_range) > 0.5 and smart_net > 0: 
            adv.append("🛡️ 探底洗盤成功，主力護盤")
        if smart_net > 50 and gap > 0: 
            adv.append("🔥 主動鎖碼/強勢推升")
        elif smart_net > 50 and gap < 0: 
            adv.append("🩹 大戶接刀/弱勢護盤")
        elif smart_net < -100 and sp > 0: 
            adv.append("📉 拉高派發/撤退")
        elif smart_net < -100 and sp <= 0: 
            adv.append("💀 波段棄守/多殺多")
        
        if eye_diag and eye_diag != "🔵 中性換手": 
            adv.append(eye_diag)
        elif not adv: 
            adv.append("🔵 盤整/無明顯特徵")
...
def process_cbas(df, current_stock_price, df_cb_info=None):
    ...
    if "可轉債代號" in df_out.columns: 
        df_out['可轉債代號'] = df_out['可轉債代號'].astype(str).str.replace(',', '', regex=False).str.replace('.0', '', regex=False).str.strip()
    ...
    if df_cb_info is not None and not df_cb_info.empty and "未償還餘額" in df_out.columns:
        ...
        if "可轉債代號" in df_cb_info_clean.columns:
            ...
            cols_to_merge = ['可轉債代號']
            if "發行總額" in df_cb_info_clean.columns: 
                cols_to_merge.append("發行總額")
            if "到期日" in df_cb_info_clean.columns: 
                cols_to_merge.append("到期日")
...
def generate_ai_hawk_eye(df_daily, df_radar, df_fingerprint, df_diff, fire_thresh):
    ...
    if not df_fingerprint.empty and len(df_fingerprint) >= 1:
        ...
        if tourists > 8: 
            alerts.append(f"<span class='hawk-alert'>⚠️ 【游擊客炸彈】前 15 大買超中，高達 {tourists} 家是低黏著度的游擊客。明日開盤 9:30 前必定湧現龐大倒貨潮，嚴禁追高！</span>")
        elif makers >= 3: 
            alerts.append(f"<span class='hawk-safe'>🔥 【潛伏主力現蹤】前 15 大分點有 {makers} 家是高黏著度的長駐大戶。波段底單深厚，具備高度安全邊際。</span>")
        else: 
    ...
    if not df_radar.empty and len(df_radar) >= 1:
        ...
        try:
            ...
            pass
    if not alerts: 
        alerts.append("<span>🔍 綜合火力與成本評估：目前籌碼結構中性，請依紀律操作。</span>")
    return alerts

_num_re = re.compile(r'\d+')
def clean_level_by_math(x):
    s = str(x).replace(',','').replace(' ','')
    if s in ["17","17.0","合計","總計"]: 
        return "合計"
    n = _num_re.findall(s)
    if not n: 
        return s
...
def process_tdcc(df):
    ...
    for l in lvls:
        if l not in p_u.columns: 
            p_u[l] = 0
        if l not in p_p.columns: 
            p_p[l] = 0
...
def process_tdcc_dynamic(df_share_wide, df_price, dead_chip_input, dynamic_dict, static_val, chip_engine):
    if df_share_wide.empty or df_price.empty: 
        return pd.DataFrame()
    ...
    for _, row in df_m.iterrows():
        ...
        if 0 < cur_dead < 100:
            ...
            cd = round(cv * 100, 2)
        out.append({"日期": row['日期'], "收盤價(元)": p, "股本(億)": round(cap, 2), "大戶精算門檻": f"系統判定 ({int(ct)}張)", "大戶原持股(%)": round(lp, 2), "董監事持股(%)": f"{float(cur_dead):.2f}% ({cl})" if cur_dead > 0 else "-", "純淨活大戶C_Value(%)": cd, "實戰判定": st_val})
    return pd.DataFrame(out)
    ...
    if df is None or df.empty: 
        ...
        def fmt_auto(x, col_name=""):
            ...
            if s in ["-", ""]: 
                return "-"
            if s.startswith("+"): 
                return f"<span style='color:#d9480f; font-weight:bold;'>{s}</span>"
            if "⚠️(虧)" in s:
                v_str = s.replace("⚠️(虧)", "").strip()
                try: 
                    return f"<span class='loss-warning'>⚠️(虧) {float(v_str.replace(',','').replace('%','')):,.2f}</span>"
                except: 
                    return f"<span class='loss-warning'>{s}</span>"
            if any(kw in col_name for kw in ["代號", "年份", "次數"]): 
                return s
            is_pct = "%" in s
            try:
        ...
        if custom_class: 
            html = html.replace('<table', f'<table class="{custom_class}"')
...
if run_btn:
    ...
    with st.spinner(f"正在啟動 V47.2 決策引擎 (相容舊有功能載入中)..."):
        ...
        if not dates: 
            st.error("⚠️ 無法取得有效交易日期，請確認 API 連線狀態。")
            st.stop()
        ...
        if not df_cbas_raw.empty and 'cb_id' in df_cbas_raw.columns:
            ...
            for cid in target_cbs:
                ...
                if not info_df.empty: 
                    df_cb_info_list.append(info_df)
        ...
        # 🎨 V47.2 頂層：紅綠燈號決策看板
        # ==========================================
        st.subheader(f"📊 {user_stock_id} {name} 全息戰報 (V47.2 動態大腦相容版)")
        ...
        if not df_ta_full.empty:
            ...
            if not df_plot.empty:
                ...
                # V47.2: 黑白 K 線，並強制設定線寬 increasing_line_width=1, decreasing_line_width=1 防止白棒變成實心黑棒
                ...
                if f'MA{ma_short}' in df_plot.columns: 
                    fig_kline.add_trace(go.Scatter(x=df_plot['日期'], y=df_plot[f'MA{ma_short}'], mode='lines', name=f'MA{ma_short}', line=dict(color='#ffa726', width=1.5)), row=1, col=1)
                if f'MA{ma_mid}(中線)' in df_plot.columns: 
                    fig_kline.add_trace(go.Scatter(x=df_plot['日期'], y=df_plot[f'MA{ma_mid}(中線)'], mode='lines', name=f'MA{ma_mid}', line=dict(color='#29b6f6', width=2)), row=1, col=1)
                if f'MA{ma_long}(長線)' in df_plot.columns: 
                    fig_kline.add_trace(go.Scatter(x=df_plot['日期'], y=df_plot[f'MA{ma_long}(長線)'], mode='lines', name=f'MA{ma_long}', line=dict(color='#ab47bc', width=2.5)), row=1, col=1)
                
                vol_colors = ['white' if row['收盤價(元)'] >= row['開盤價(元)'] else 'black' for i, row in df_plot.iterrows()]
                fig_kline.add_trace(go.Bar(
                    x=df_plot['日期'], y=df_plot['成交量(張)'], 
                    marker=dict(color=vol_colors, line=dict(color='black', width=1)), 
                    name='成交量'
                ), row=2, col=1)
        ...
        # ==========================================
        # 🔍 V47.2 新增：底層數據 Raw Data Dump 驗證區
        # ==========================================
        st.divider()
        st.markdown("<div class='category-title'>🔍 系統底層數據 Raw Data Dump 驗證區</div>", unsafe_allow_html=True)
        with st.expander("點此展開系統原始擷取數據 (供驗證 00, 01 等模組計算邏輯)", expanded=False):
            st.write("### 1. 股價原始數據 df_p_raw (前 5 筆)")
            st.dataframe(df_p_raw.head(5) if not df_p_raw.empty else pd.DataFrame(), use_container_width=True)
            
            st.write("### 2. 集保股權分散原始數據 df_s_raw (前 15 筆)")
            st.dataframe(df_s_raw.head(15) if not df_s_raw.empty else pd.DataFrame(), use_container_width=True)
            
            st.write("### 3. 分點進出原始數據 df_b_raw (前 15 筆)")
            st.dataframe(df_b_raw.head(15) if not df_b_raw.empty else pd.DataFrame(), use_container_width=True)
            
            st.write("### 4. 董監事持股動態字典 (Goodinfo 抓取結果)")
            st.json(dynamic_dict)
