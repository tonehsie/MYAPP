...
# 設定網頁標題與佈局
st.set_page_config(page_title="V26.9 終極全息量化系統", layout="wide")
...
st.title("🤖 交易員實戰手冊：V26.9 全息量化除水系統")
st.caption("核心修復：還原鉅額交易防呆避震器，徹底解決 ValueError 當機問題。")

# 內建字典 (展開式)
with st.expander("📖 【必讀】V26.9 獨家數據欄位與實戰名詞解釋", expanded=False):
    st.markdown("""
    <div class='dict-box'>
...
run_btn = st.button("🚀 啟動 V26.9 引擎：擷取地緣情報與全息雷達", use_container_width=True)
st.divider()
...
def scrape_block_trades(target_id, actual_dates):
    ...
    for item in block_data:
        date, src, row = item
        nums = []
        # ⚠️ 【關鍵修復點】：還原被刪除的 try...except，避免 ValueError 當機
        for c in row:
...
def process_v25_ultimate_radar(df_wide, dead_chip_input, dynamic_dict, static_val, df_price, df_branch_raw, intel_tags):
    ...
    df['純淨大戶變動(%)'], df['隔日沖虛胖(%)'], df['V26.9_雷達診斷'] = ddf['純淨變動'], ddf['雜訊'], ddf['診斷']
    return df[['日期', '收盤價(元)', '總人數變動率(%)', '原始大戶變動(%)', '純淨大戶變動(%)', '隔日沖虛胖(%)', 'V26.9_雷達診斷']].sort_values('日期', ascending=False), pd.DataFrame(d_math), pd.DataFrame(d_fri)
...
# ==========================================
if run_btn:
    with st.spinner(f"正在執行 V26.9 終極全息引擎 (台積電防呆修復版)..."):
        ...
        # --- 頁面呈現 ---
        st.subheader(f"📊 {user_stock_id} {name} V26.9 全息戰報")
        ...
        show_table("1-2. V26.9 專家診斷雷達 (週末除水版) (近8週)", df_v26_radar, "radar-table")
        ...
        st.divider()
        with st.expander("🛠️ 【開發者專用】V26.9 演算法稽核中心", expanded=False):
        ...
        st.divider()
        with st.expander("📋 【點擊展開：給 Gemini 的 V26.9 量化分析與稽核資料包 (CSV格式)】", expanded=True):
            p = f"請分析標的: {user_stock_id} {name} (V26.9 量化籌碼)\n"
            ...
            p += format_to_csv_string(df_v26_radar.head(8), "1-2. V26.9 專家診斷雷達 (週末除水版)")
            ...
            p += "\n請幫我結合『V26.9 名詞定義』與上列數據，給出最新的實戰操作建議。"
