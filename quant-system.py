...
# 設定網頁標題與佈局
st.set_page_config(page_title="V26.6 終極全息量化系統", layout="wide")
...
st.title("🤖 交易員實戰手冊：V26.6 全息量化除水系統")
st.caption("核心升級：強制數學精算集保比例，徹底排除 API 比例失真 Bug。")

# UI 輸入區
col1, col2 = st.columns([1, 1])
with col1: user_stock_id = st.text_input("個股代號", value="8027")
with col2: dead_chip_input = st.text_input("死籌碼 %", placeholder="留空自動計算")
run_btn = st.button("🚀 啟動 V26.6 引擎：擷取地緣情報與全息雷達", use_container_width=True)
st.divider()
...
def process_v25_ultimate_radar(df_wide, dead_chip_input, dynamic_dict, static_val, df_price, df_branch_raw, intel_tags):
    ...
    for i, row in df.iterrows():
        if pd.isna(row['1000張變動(%)']): out.append({"真實變動": 0, "雜訊": 0, "診斷": "⚪ 初始化"}); continue
        d_str = row['日期']
        ...
        p_chg = round(row['1000張變動(%)'] - f_impact, 2)
        d_math.append({"日期": d_str, "原始變動": row['1000張變動(%)'], "隔日沖干擾": round(f_impact, 2), "純淨變動": p_chg})
    ...
    df['真實大戶變動(%)'], df['隔日沖雜訊(%)'], df['V26.6_專家診斷'] = ddf['真實變動'], ddf['雜訊'], ddf['診斷']
    return df[['日期', '收盤價(元)', '總人數變動率(%)', '1000張變動(%)', '真實大戶變動(%)', '隔日沖雜訊(%)', 'V26.6_專家診斷']].sort_values('日期', ascending=False), pd.DataFrame(d_math), pd.DataFrame(d_fri)
...
def process_v26_daily_tracking(df_branch_raw, intel_tags, df_price, df_branch_diff, actual_dates):
    ...
    for d in actual_dates[:5]:
        ...
        elif sn < -100: adv.append("🔴 聰明錢撤退")
        if nn > 300: adv.append("⚠️ 隔日沖進駐")
        if bsd > 100: adv.append("📉 散戶進場/籌碼發散")
        out.append({"日期": d, "收盤價(元)": cp, "漲跌(元)": sp, "聰明錢淨流(張)": int(sn), "隔日沖留倉(張)": int(nn), "買賣家數差": bsd, "平日獨立診斷": " | ".join(adv) if adv else "無明顯特徵"})
    return pd.DataFrame(out)

# ==========================================
# 📌 資料處理與排版 (【核心修復區】：徹底廢棄 API 比例，改用自算引擎)
# ==========================================
...
def clean_level_by_math(x):
    ...
    else: return "1000張以上" # 強制修復：超過800大關的剩餘通通歸入千張大戶
...
def process_tdcc(df):
    ...
    # 【核心修復】：改用 sum 來避免同級距資料遺漏，並完全捨棄 API 的 percent
...
def process_tdcc_dynamic(df_share_wide, df_price, dead_chip_input, dynamic_dict, static_val, chip_engine):
    ...
    for _, row in df_m.iterrows():
        ...
        if 0 < cur_dead < 100:
            ...
            cd = round(cv * 100, 2)
        out.append({"日期": row['日期'], "收盤價(元)": p, "股本(億)": round(cap, 2), "主導門檻": f"智能精算 ({int(ct)}張)", "級距總佔比(%)": round(lp, 2), "死籌碼(%)": f"{float(cur_dead):.2f}% ({cl})" if cur_dead > 0 else "-", "活大戶C_Value(%)": cd, "實戰判定": st})
    return pd.DataFrame(out)
...
# ==========================================
if run_btn:
    with st.spinner(f"正在執行 V26.6 全息除水引擎 (數學自算版)..."):
        ...
        # --- 頁面呈現 ---
        st.subheader(f"📊 {user_stock_id} {name} V26.6 全息戰報")
        ...
        show_table("1-2. V26.6 專家診斷雷達 (週末除水版) (近8週)", df_v26_radar, "radar-table")
        ...
        st.divider()
        with st.expander("🛠️ 【開發者專用】V26.6 演算法稽核中心", expanded=False):
        ...
        st.divider()
        with st.expander("📋 【點擊展開：給 Gemini 的 V26.6 量化分析與稽核資料包 (CSV格式)】", expanded=True):
            p = f"請分析標的: {user_stock_id} {name} (V26.6 量化籌碼)\n"
            ...
            p += format_to_csv_string(df_v26_radar.head(8), "1-2. V26.6 專家診斷雷達 (週末除水版)")
