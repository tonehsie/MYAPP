...
st.set_page_config(page_title="全息量化系統 (V60.20版)", layout="wide", initial_sidebar_state="expanded")
...
st.title("全息量化系統 (V60.20 TradingView 極速版)")
user_count, api_limit = get_api_usage(FINMIND_TOKEN)
usage_text = f" | FinMind 額度: {user_count} / {api_limit}" if user_count is not None else ""
st.caption(f"系統核心：Lightweight Charts 渲染與純淨演算法模式{usage_text}")
...
def clean_level_by_math(x):
    ...
    elif u<=20000: return "100-200張"
    elif u<=40000: return "200-400張"
...
if run_btn:
    ...
    with st.spinner(f"正在啟動 V60.20 決策引擎 (TradingView 極速圖表建構中)..."):
        ...
        if not df_ta_full.empty:
            ...
            def prep_ma(df, col):
                ...
                s = df[['日期', col]].dropna()
                return [{'time': str(t), 'value': round(float(v), 2)} for t, v in zip(s['日期'], s[col])]
            ...
            if not df_lr_channel.empty:
                ...
                lr_data["upper"] = [{'time': str(t), 'value': float(v)} for t, v in zip(df_lr_sorted['日期'], df_lr_sorted['LR_Upper'])]
                lr_data["mid"] = [{'time': str(t), 'value': float(v)} for t, v in zip(df_lr_sorted['日期'], df_lr_sorted['LR_Mid'])]
                lr_data["lower"] = [{'time': str(t), 'value': float(v)} for t, v in zip(df_lr_sorted['日期'], df_lr_sorted['LR_Lower'])]

            pattern_lines = []
            if pat_data:
                shape_line = [{'time': str(t), 'value': float(v)} for t, v in zip(pat_data['shape_x'], pat_data['shape_y'])]
                neck_line = [{'time': str(t), 'value': float(v)} for t, v in zip(pat_data['neck_x'], pat_data['neck_y'])]
                pattern_lines = [
                    {"data": shape_line, "color": pat_data['color'], "lineWidth": 4, "lineStyle": 0},
                    {"data": neck_line, "color": pat_data['color'], "lineWidth": 2, "lineStyle": 2}
                ]
        ...
        if bias > 10 and net_60 > 0: layer1_diag = f"長線大戶建立利潤墊(乖離>10%)，具備波段趨勢啟動特徵，{lr_pos_text}，下檔底單保護強固。"
        ...
        with st.expander(f"給 Gemini 的 V60.20 實戰精華資料包 (CSV格式)", expanded=True):
