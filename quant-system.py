...
# 設定網頁標題與佈局
st.set_page_config(page_title="V33.4 終極全息量化系統 (120天K線無斷層版)", layout="wide")
...
st.title("📱 V33.4 終極全息量化系統 (120天K線無斷層版)")
st.caption("圖表優化：強制 X 軸為類別屬性，徹底消除假日空白斷層，並精簡為純 K 線與成交量。")
...
run_btn = st.button("🚀 啟動 V33.4 完美無瑕運算引擎", use_container_width=True)

with st.expander("📖 【V33.4 實戰字典：破解主力的最高機密】", expanded=False):
    st.markdown("""
    <div class='dict-box'>
...
def generate_ai_hawk_eye(df_daily, df_radar, df_fingerprint, df_diff):
    ...
    if not df_fingerprint.empty and len(df_fingerprint) >= 1:
        ...
        elif locked >= 2 and locked > trapped: alerts.append(f"<span class='hawk-safe'>🔥 【主動鎖碼強勢】前 15 大分點有 {locked} 家處於『獲利強勢留倉』狀態，主力買均價極具防守優勢，具波段續攻潛力。</span>")
...
def plot_interactive_kline(df_price, df_ta):
    ...
    df_p = df_price.head(120).copy()
    df_t = df_ta.head(120).copy()
    if '收盤價(元)' in df_t.columns: df_t = df_t.drop(columns=['收盤價(元)'])
    
    df = pd.merge(df_p, df_t, on='日期', how='inner').sort_values('日期', ascending=True)
    df['日期'] = df['日期'].astype(str) # 轉字串消除斷層
    
    fig = make_subplots(rows=2, cols=1, shared_xaxes=True, vertical_spacing=0.03, row_heights=[0.75, 0.25], subplot_titles=("K線與均線 (120天)", "成交量(張)"))
    ...
    fig.update_layout(height=650, margin=dict(l=30, r=30, t=40, b=30), xaxis_rangeslider_visible=False, plot_bgcolor='#f8f9fa', paper_bgcolor='white', legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="right", x=1))
    
    fig.update_xaxes(type='category', showgrid=True, gridwidth=1, gridcolor='#e0e0e0', tickangle=45, row=2, col=1)
...
if run_btn:
    ...
    with st.spinner(f"正在啟動 V33.4 視覺化淨化引擎..."):
        ...
        df_ta_display = df_ta_full[ta_display_cols].copy() if not df_ta_full.empty else pd.DataFrame()
        ...
        st.subheader(f"📊 {user_stock_id} {name} 全息戰報 (V33.4 視覺化淨化版)")
        ...
        if not df_ta_display.empty:
            ...
            st.markdown("<div class='section-title'>📈 04-2. 互動式 K 線與成交量 (近120日)</div>", unsafe_allow_html=True)
        ...
        with st.expander(f"📋 給 Gemini 的 V33.4 實戰精華資料包 (CSV格式)", expanded=True):
        ...
        with st.expander(f"🔎 給 Gemini 的 V33.4 稽核與驗算資料包 (CSV格式)", expanded=False):
