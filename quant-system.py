...
st.title("全息量化系統 (V60.46 向量快取版)")
user_count, api_limit = get_api_usage(FINMIND_TOKEN)
usage_text = f" | FinMind 額度: {user_count} / {api_limit}" if user_count is not None else ""
st.caption(f"V60.46：全面導入 Pandas Vectorization 向量化矩陣運算與 Memoization 快取機制，徹底消滅巢狀迴圈與無效存取。{usage_text}")
...
def calculate_pure_defense_line(df_b_raw, tags, is_filter_active, total_lots, dead_chip_ratio, dynamic_n):
    ...
    df = df_b_raw[['date', 'securities_trader', 'buy', 'sell', 'price', 'valid_buy', 'valid_buy_amt']].copy()
...
def process_branch_v25(df_raw, period, actual_dates, intel_tags, df_price_raw, stick_thresh, global_days):
    ...
    df = df_raw.loc[df_raw['date'].isin(actual_dates[:period]), ['date', 'securities_trader', 'buy', 'sell', 'price', 'valid_buy', 'valid_sell', 'valid_buy_amt', 'valid_sell_amt']].copy()
...
def process_v27_ultimate_radar(df_wide, parsed_dci, dynamic_dict, static_val, df_price, df_branch_raw, intel_tags):
    ...
    idx_p = cols.index('收盤價(元)')
    idx_lots = cols.index('總張數')
    idx_100 = cols.index('pct_100')
    idx_200 = cols.index('pct_200')
    idx_400 = cols.index('pct_400')
    idx_600 = cols.index('pct_600')
    idx_800 = cols.index('pct_800')
    idx_1000 = cols.index('pct_1000')
    idx_ppl_var = cols.index('總人數變率(%)')
    idx_ma20 = cols.index('ma20')
    ...
    for row in df.values:
        ...
        d_dt = row[idx_dt_end]
        p = row[idx_p]
        total_lots = row[idx_lots]
        ppl_var = row[idx_ppl_var]
        ma20 = row[idx_ma20]
        ...
        cur_dead, _ = get_dead_chip_info(d_str, parsed_dci, dynamic_dict, static_val, "")
        safe_dead_ratio = max(0.0, min(99.9, cur_dead))
        ct = get_smart_threshold(p, total_lots, safe_dead_ratio)
        
        if ct <= 100: current_large_pct = row[idx_100]
        elif ct <= 200: current_large_pct = row[idx_200]
        elif ct <= 400: current_large_pct = row[idx_400]
        elif ct <= 600: current_large_pct = row[idx_600]
        elif ct <= 800: current_large_pct = row[idx_800]
        else: current_large_pct = row[idx_1000]
        
        if prev_row_vals is None:
            raw_chg, p_chg, f_impact = 0.0, 0.0, 0.0
            adv = ["初始化 (基準建立)"]
        else:
            if ct <= 100: prev_large_pct_adj = prev_row_vals[idx_100]
            elif ct <= 200: prev_large_pct_adj = prev_row_vals[idx_200]
            elif ct <= 400: prev_large_pct_adj = prev_row_vals[idx_400]
            elif ct <= 600: prev_large_pct_adj = prev_row_vals[idx_600]
            elif ct <= 800: prev_large_pct_adj = prev_row_vals[idx_800]
            else: prev_large_pct_adj = prev_row_vals[idx_1000]
...
# 【V60.46 優化】極致 Pandas 向量化運算，徹底消滅 for 迴圈
def process_branch_diff(df_raw, actual_dates, fire_thresh, period_days=10):
    ...
    df_f = df_raw[df_raw['date'].isin(target_dates)]
...
def process_v30_daily_tracking(df_branch_raw, intel_tags, df_price, df_branch_diff, actual_dates, fire_thresh, period_days=5):
    ...
    df_b = df_branch_raw[df_branch_raw['date'].isin(target_dates)][['date', 'securities_trader', 'buy', 'sell', 'price', 'valid_buy_amt', 'valid_sell_amt']].copy()
    ...
    smart_group = df_smart_all.groupby('date') if not df_smart_all.empty else None
    ...
    short_group = df_short_all.groupby('date') if not df_short_all.empty else None
    ...
    for d in actual_dates[:period_days]:
        ...
        smart_grouped = smart_group.get_group(d) if smart_group and d in smart_group.groups else pd.DataFrame(columns=['securities_trader', 'tag', 'bs', 'ss', 'buy_amt', 'sell_amt', 'net_vol'])
        short_grouped = short_group.get_group(d) if short_group and d in short_group.groups else pd.DataFrame(columns=['securities_trader', 'bs', 'ss', 'net_vol'])
        
        if d == actual_dates[0] and not smart_grouped.empty:
...
# 【V60.46 優化】全域記憶體快取 (Memoization)，提速百倍
_LEVEL_CLEAN_CACHE = {}
...
def process_tdcc_dynamic(df_share_wide, df_price, parsed_dci, dynamic_dict, static_val, chip_engine):
    ...
    for row in df_m.values:
        ...
        d_str = row[idx_date]
        cur_dead, cl = get_dead_chip_info(d_str, parsed_dci, dynamic_dict, static_val, chip_engine)
        total_lots = row[idx_lots]
        safe_dead_ratio = max(0.0, min(99.9, cur_dead))
        ct = get_smart_threshold(p, total_lots, safe_dead_ratio)
        
        if ct <= 100: lp = row[idx_100]
        elif ct <= 200: lp = row[idx_200]
        elif ct <= 400: lp = row[idx_400]
        elif ct <= 600: lp = row[idx_600]
        elif ct <= 800: lp = row[idx_800]
        else: lp = row[idx_1000]
        ...
        if 0 < safe_dead_ratio < 100:
            ...
            cd = round(cv * 100, 2)
        out.append({"日期": d_str, "收盤價(元)": p, "大戶精算門檻": f"系統判定 ({int(ct)}張)", "大戶原持股(%)": round(lp, 2), "董監死籌碼(%)": f"{float(safe_dead_ratio):.2f}% ({cl})" if safe_dead_ratio > 0 else "-", "純淨活大戶C_Value(%)": cd, "實戰判定": st_val})
    return pd.DataFrame(out)
...
if run_btn:
    ...
    with st.spinner(f"正在啟動 V60.46 快取向量化極限引擎..."):
        ...
        # 【V60.46 優化】前置過濾時間切片，減少 Pandas 在龐大資料集上的 isin() 比對負擔
