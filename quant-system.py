...
def process_tdcc(df):
    ...
    p_u = df_levels.pivot_table(index='date', columns='LevelClean', values='unit', aggfunc='first').reset_index().fillna(0)
    p_p = df_levels.pivot_table(index='date', columns='LevelClean', values='people', aggfunc='first').reset_index().fillna(0)
    p_c = df_levels.pivot_table(index='date', columns='LevelClean', values='percent', aggfunc='first').reset_index().fillna(0)
    ...
    for l in lvls:
        ...
        if l not in p_c.columns: p_c[l] = 0

    df_t = pd.DataFrame({'date': p_u['date']})
    df_t['總張數'], df_t['總人數(人)'] = p_u[lvls].sum(axis=1), p_p[lvls].sum(axis=1)
    df_w = df_t.copy()
    for l in lvls: df_w[f"{l}_張數"], df_w[f"{l}_人數"], df_w[f"{l}_比例(%)"] = p_u[l], p_p[l], p_c[l]
