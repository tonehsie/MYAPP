...
st.title("📱 全息量化系統 (V60.05 系統防護版)")
user_count, api_limit = get_api_usage(FINMIND_TOKEN)
usage_text = f" | 🔑 FinMind 額度: {user_count} / {api_limit}" if user_count is not None else ""
st.caption(f"🚀 V60.05：修正 K 線十字線貼合收盤價，強化鷹眼引擎底層防錯機制。{usage_text}")
...
def get_v50_intelligence(df_b_raw, df_p_raw, stick_thresh, global_days, dates_list):
    ...
    cond_gov = g.index.str.contains('|'.join(gov_list), na=False)
    cond_daytrade = (g['net_60d'].between(-200, 200)) & (g['net_20d'].between(-200, 200)) & (g['net_5d'] >= 300)
    cond_dump = (g['net_60d'] >= 300) & (g['net_20d'] >= 100) & (g['net_5d'] <= -100)
    cond_long = (g['net_60d'] >= 200) & (g['net_20d'] >= 100) & (g['net_5d'] >= 50)
    cond_short = (g['net_60d'] <= -200) & (g['net_20d'] <= -100) & (g['net_5d'] <= -100)
    cond_stuck = (g['net_60d'] <= -100) & (g['net_5d'] >= 200)
    cond_maker = g['stickiness'] >= stick_thresh
    cond_tourist = (g['stickiness'] < 10.0) & (g['net_5d'].abs() > 50)

    g['tag'] = np.select(
        [cond_gov, cond_daytrade, cond_dump, cond_long, cond_short, cond_stuck, cond_maker, cond_tourist],
        ["🏦 [影子官股]", "⚠️ [隔日沖大戶]", "💀 [大戶出貨]", "🔥 [長駐波段主]", "📉 [趨勢空方]", "🩹 [被套牢]", "🥷 [潛伏造市者]", "🏃 [游擊過客]"],
        default="🔵 一般/游擊"
    )
...
def calculate_pure_defense_line(df_b_raw, tags, is_filter_active, total_lots, dead_chip_ratio, dynamic_n):
    ...
    if is_filter_active: 
        valid_df = df[~df['tag'].str.contains("隔日沖|游擊|空方", na=False)].copy()
    else: 
        valid_df = df
...
# V60.05 加入極強的 try-except 錯誤捕獲，防止空值或型別錯誤導致 line 1468 報錯當機
def generate_ai_hawk_eye(df_daily, df_radar, df_fingerprint, df_diff, fire_thresh):
    ...
    try:
        if not df_daily.empty and len(df_daily) >= 1:
            ...
            gap_valid = pd.notna(gap_val_raw) and str(gap_val_raw).strip() != "-"
            
            if gap_valid:
                try:
                    gap_val = float(str(gap_val_raw).replace(',', '').strip())
                    chg_raw = today_d.get('漲跌(元)', 0)
                    chg_val = float(str(chg_raw).replace(',', '').strip()) if str(chg_raw) not in ["-", ""] else 0.0
                    
                    smart_net_val = float(smart_net)
                    if gap_val > 0 and smart_net_val > 0: alerts.append(f"> 🟢 **【主動鎖碼】** {flow_str} 大戶買進均價低於收盤價 (落差 +{gap_val})。主力具備強勢推升意願。")
                    elif gap_val < 0 and smart_net_val > 0: alerts.append(f"> 🔴 **【接刀套牢】** {flow_str} 大戶買進均價高於收盤價 (落差 {gap_val})。主力護盤已被套牢，易引發停損賣壓！")
                    elif smart_net_val < -100 and chg_val > 0: alerts.append(f"> 🔴 **【拉高派發】** 今日股價收紅，聰明錢卻趁機撤退。主力逢高倒貨，追高風險大。")
                    elif smart_net_val < -100: alerts.append(f"> 💀 **【波段棄守】** 股價走弱且聰明錢大舉撤退。長線防守線可能崩潰。")
        ...
        alerts.append(f"> ⚠️ 鷹眼系統解析異常，部分數值目前無資料。")
...
if run_btn:
    ...
    with st.spinner(f"正在啟動 V60.05 決策引擎 (鷹眼系統保護層已啟動)..."):
        ...
        if not df_ta_full.empty:
            ...
            if not df_plot.empty:
                ...
                # V60.05 完美修復十字線：使用 closest 模式並確保 XY 都加上 showspikes，spikesnap='data' 確保貼合收盤價
                fig.update_layout(height=650, margin=dict(l=30, r=30, t=20, b=20), xaxis_rangeslider_visible=False, plot_bgcolor='white', paper_bgcolor='white', hovermode='closest', showlegend=False)
                fig.update_xaxes(showgrid=False, zeroline=False, type='category', showspikes=True, spikemode='toaxis+across', spikethickness=1, spikecolor='#999999', spikedash='solid', spikesnap='data', row=1, col=1)
                fig.update_yaxes(showgrid=True, gridcolor='#f0f0f0', zeroline=False, showspikes=True, spikemode='toaxis+across', spikethickness=1, spikecolor='#999999', spikedash='solid', spikesnap='data', row=1, col=1)
                fig.update_xaxes(showgrid=False, zeroline=False, tickangle=45, type='category', showspikes=True, spikemode='toaxis+across', spikethickness=1, spikecolor='#999999', spikedash='solid', spikesnap='data', row=2, col=1)
        ...
        render_clean_html_table(df_gov, "04. 影子官股進出 (今日)")
        render_clean_html_table(df_inst, "05. 法人買賣超 (近10天)")
        render_clean_html_table(df_margin, "06. 散戶資券餘額 (近10天)")
        ...
        with st.expander("📂 10. 點此展開集保分級表 (近8週)", expanded=False):
        ...
        with st.expander("📂 12. 點此展開董監大股東質設明細", expanded=False):
            render_clean_html_table(df_p_det, "12. 董監大股東質設明細")
            
        render_clean_html_table(df_twse, "13. 鉅額交易明細 (近3天)")
        render_clean_html_table(df_div, "14. 歷年股利政策 (近5年)")
        render_clean_html_table(df_per, "15. 本益比、淨值比與殖利率")
        ...
        with st.expander(f"📋 給 Gemini 的 V60.05 實戰精華資料包 (CSV格式)", expanded=True):
            ...
            p1 += format_to_csv_string(df_inst.head(10) if not df_inst.empty else df_inst, "05. 法人買賣超 (近10天)")
            p1 += format_to_csv_string(df_margin.head(10) if not df_margin.empty else df_margin, "06. 散戶資券餘額 (近10天)")
            p1 += format_to_csv_string(df_day_trade.head(10) if not df_day_trade.empty else df_day_trade, "07. 現股當沖明細 (近10天)")
            p1 += format_to_csv_string(df_fut.head(10) if not df_fut.empty else df_fut, "08. 台指期貨三大法人未平倉 (大盤)")
            p1 += format_to_csv_string(df_rev.head(12) if not df_rev.empty else df_rev, "09. 月營收 (百萬元) (近12個月)")
            p1 += format_to_csv_string(df_p_sum, "11. 董監大股東質設總覽")
            p1 += format_to_csv_string(df_twse, "13. 鉅額交易明細 (近3日)")
            p1 += format_to_csv_string(df_per.head(10) if not df_per.empty else df_per, "15. 本益比、淨值比與殖利率")
