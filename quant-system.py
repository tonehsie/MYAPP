...
st.set_page_config(layout="wide", page_title="全息量化系統 (V60.51版)", initial_sidebar_state="expanded")
...
st.title("全息量化系統 (V60.51 終極完整極速版)")
user_count, api_limit = get_api_usage(FINMIND_TOKEN)
usage_text = f" | FinMind 額度: {user_count} / {api_limit}" if user_count is not None else ""
st.caption(f"V60.51：完整修復遺失的技術分析模組，保留最強的集保「以上」快取過濾與全矩陣向量化引擎。{usage_text}")
...
run_btn = st.button("啟動 V60.51 決策引擎", use_container_width=True, key="run_engine")
...
# 補回遺漏的 process_price, process_technical_analysis, process_linear_regression, process_geometric_patterns
...
if run_btn:
    ...
    with st.spinner(f"正在啟動 V60.51 終極修復決策引擎..."):
        ...
        st.subheader(f"{user_stock_id} {name} 全息戰報 (V60.51)")
        ...
        with st.expander(f"給 AI 的 V60.51 實戰精華資料包 (CSV格式)", expanded=True):
