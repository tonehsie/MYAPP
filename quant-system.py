...
# 設定網頁標題與佈局
st.set_page_config(page_title="V31.0 終極全息量化系統 (完美對帳版)", layout="wide")
...
st.title("📱 V31.0 終極全息量化系統 (完美對帳版)")
st.caption("稽核修復：全面修正 Python 零股強制捨去 (Truncation) 導致的總和飄移，確保矩陣與明細 100% 吻合。")
...
run_btn = st.button("🚀 啟動 V31.0 完美無瑕運算引擎", use_container_width=True)

# 內建字典
with st.expander("📖 【V31.0 實戰字典：破解主力的最高機密】", expanded=False):
    st.markdown("""
    <div class='dict-box'>
...
if run_btn:
    ...
    with st.spinner(f"正在啟動 V31.0 完美對帳運算引擎..."):
        ...
        # ⚠️ 頁面呈現
        # ==========================================
        st.subheader(f"📊 {user_stock_id} {name} 全息戰報 (V31.0 完美對帳版)")
        ...
        with st.expander(f"📋 給 Gemini 的 V31.0 實戰精華資料包 (CSV格式)", expanded=True):
        ...
        with st.expander(f"🔎 給 Gemini 的 V31.0 稽核與驗算資料包 (CSV格式)", expanded=False):
