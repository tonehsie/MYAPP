...
st.title("全息量化系統 (V60.42 磐石修復版)")
user_count, api_limit = get_api_usage(FINMIND_TOKEN)
usage_text = f" | FinMind 額度: {user_count} / {api_limit}" if user_count is not None else ""
st.caption(f"V60.42：完整還原並強化智慧型集保解析引擎，徹底解決級距丟失與資料歸零問題。{usage_text}")
...
def process_tdcc(df):
    ...
    # 消除 Fragmentation 警告，採用 C 級 .reindex() 瞬間對齊欄位
...
if run_btn:
    ...
    with st.spinner(f"正在啟動 V60.42 決策引擎..."):
