...
st.sidebar.divider()
run_btn = st.sidebar.button("🚀 啟動全市場集保掃描", use_container_width=True)

# ==========================================
# 主畫面與掃描邏輯
# ==========================================
st.title("全息量化系統 (V76.6 贊助會員專屬光速雷達)")
st.caption("已啟動 FinMind 贊助會員專屬 API，直接取得單日全市場快照，完美迴避迴圈請求限制。")
...
if run_btn:
    ...
    with st.spinner(f"正在行使 Sponsor 權限... 向 FinMind 請求全市場 {latest_date} 與 {prev_date} 的集保數據..."):
        ...
        if df_latest.empty or df_prev.empty:
            st.error("全市場集保數據抓取失敗。請確認您的 Token 是否仍具備 Sponsor 資格。")
            st.stop()
    ...
    with st.spinner("資料解析中... 進行股本過濾與大戶增減計算..."):
        ...
        def process_market_snapshot(df_snap):
            # 💡 核心修正：自動判斷 API 回傳的欄位名稱是 unit 還是 HoldingShares
            val_col = 'unit' if 'unit' in df_snap.columns else 'HoldingShares'
            
            if val_col not in df_snap.columns:
                st.error(f"資料表缺乏所需欄位，目前可用欄位：{df_snap.columns.tolist()}")
                return pd.DataFrame()
                
            df_snap[val_col] = pd.to_numeric(df_snap[val_col], errors='coerce').fillna(0)
    ...
    st.success(f"光速掃描完畢！共抓出 {len(df_out)} 檔符合「股本 <= {capital_limit} 億」且「大戶單週增加 >= {diff_threshold}%」的個股。")
    render_clean_html_table(df_out, "🚨 聰明錢大戶突擊雷達結果")
    
else:
    st.info("請設定左側參數，並點擊「🚀 啟動全市場集保掃描」開始執行。")
